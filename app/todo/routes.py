...
from .utils import get_current_user_id, get_todo_or_404, validate_todo_form, get_todos_by_quadrant
...
def index():
    ...
    print(f"TODO INDEX ROUTE CALLED - Endpoint: {request.endpoint}, Blueprint: {request.blueprint}")  # Debug
    ...
    print(f"Quadrant todos keys: {list(quadrant_todos.keys())}")  # Debug
    for q in [1,2,3,4]:
        print(f"  Quadrant {q}: {len(quadrant_todos.get(q, []))} todos")
